...
\section{Accessing CoreIOT via Web Server}

To provide a seamless user experience, we integrated a direct link to the CoreIOT dashboard within our local web server interface. On the home page of the device's web interface (detailed in Chapter 5), we added a dedicated "card" that redirects users to the cloud dashboard.

This integration allows users to easily transition from local control (Access Point mode) to remote monitoring (Cloud Dashboard) without needing to manually type the URL.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{assets/webserver_coreiot_link.png}
    \caption{Web server home page showing the CoreIOT access card.}
...
In our current deployment, the alarm table shows multiple \textit{Environment Warning} entries triggered on December 3, 2025. These alarms are generated when temperature exceeds 33\textdegree C, drops below 15\textdegree C, or when humidity falls outside the 40--70\% comfort range. The severity levels are color-coded: green for acknowledged alarms, yellow for warnings, and red for critical events.

We configured the alarm rules using ThingsBoard's Rule Chain editor, which provides a visual programming interface for defining conditional logic. For example, the temperature threshold rule consists of a filter node that evaluates \texttt{temperature > 33}, connected to an action node that creates an alarm entity with severity \textit{Warning} and type \textit{Environment Warning}. The rule chain also sends WebSocket notifications to the dashboard, triggering real-time alert pop-ups.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{assets/rule_chain.png}
    \caption{Rule Chain configuration for alarm processing and notification.}
