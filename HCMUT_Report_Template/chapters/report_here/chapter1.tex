...
This chapter documents the process of setting up the Core IOT platform for our IoT project. We cover account creation, device registration, and dashboard configuration.

\section{Create Core IOT Account}
...
In our firmware, we defined global variables to store the connection credentials. These values are loaded from non-volatile storage during boot, allowing users to configure them through our web interface without modifying the source code:
...
The connection to the Core IOT server is established using the ThingsBoard Arduino SDK. We initialize the MQTT client and ThingsBoard object as follows:
...
The reconnection logic handles authentication and subscription setup. When the device connects successfully, it sends its MAC address as an attribute and subscribes to RPC callbacks:
...
After configuring the device credentials, we developed a firmware routine to format sensor data into a JSON structure compliant with the Core IOT platform's requirements. We defined a struct to hold all sensor values in a thread-safe manner:
...
\subsection{Alarm Management System}

We integrated a comprehensive alarm table widget in the lower section of the dashboard to track environmental anomalies and system events. The alarm system operates through rule chains configured in the ThingsBoard Rule Engine, which continuously evaluate incoming telemetry against predefined threshold conditions.

The alarm table displays several critical columns:

\begin{itemize}
    \item \textbf{Created time:} Timestamp when the alarm was triggered, enabling chronological incident analysis
    \item \textbf{Originator:} The entity that generated the alarm (typically \textit{Assignment} for our configured rule chain)
    \item \textbf{Type:} Classification of the alarm event (e.g., \textit{Environment Warning})
    \item \textbf{Severity:} Criticality level (Warning, Critical, or Fire Alert)
    \item \textbf{Status:} Current state of the alarm (Active, Acknowledged, Cleared, Unacknowledged)
    \item \textbf{Assignee:} User or group responsible for incident response
\end{itemize}

In our current deployment, the alarm table shows multiple \textit{Environment Warning} entries triggered on December 3, 2025. These alarms are generated when temperature exceeds 30\textdegree C, drops below 20\textdegree C, or when humidity falls outside the 40--70\% comfort range. The severity levels are color-coded: green for acknowledged alarms, yellow for warnings, and red for critical events.

We configured the alarm rules using ThingsBoard's Rule Chain editor, which provides a visual programming interface for defining conditional logic. For example, the temperature threshold rule consists of a filter node that evaluates \texttt{temperature > 30}, connected to an action node that creates an alarm entity with severity \textit{Warning} and type \textit{Environment Warning}. The rule chain also sends WebSocket notifications to the dashboard, triggering real-time alert pop-ups.

The alarm table supports interactive management features. Operators can click individual alarms to view detailed information, acknowledge events to indicate they are being addressed, or assign alarms to specific team members for resolution. This functionality is particularly valuable in multi-user deployments where incident response requires coordination among maintenance personnel.

\subsection{Geographical Fleet Tracking}

At the bottom of the dashboard, we incorporated an Open Street Map widget to provide spatial context for device deployments. While our current system operates with a single device, the map widget demonstrates scalability for distributed sensor networks across multiple locations.

The map configuration requires telemetry data to include \texttt{latitude} and \texttt{longitude} fields. These coordinates can be hardcoded in the firmware for fixed installations or dynamically obtained from GPS modules for mobile deployments. Each device appears as a marker on the map, and clicking a marker displays a tooltip containing key telemetry values such as current temperature, humidity, and device status.

We customized the map widget's datasource settings to ensure proper data key mapping when multiple devices are present. By default, ThingsBoard appends numeric suffixes to duplicate data keys (e.g., \texttt{latitude\_1}, \texttt{latitude\_2}), but we configured explicit aliases to maintain consistent key names across all devices. This prevents widget configuration errors that would occur if the dashboard expected \texttt{latitude} but received \texttt{latitude\_1}.
...
In this chapter, we successfully set up the Core IOT platform for our project. We created a Tenant Administrator account, registered our ESP32 device, and established MQTT communication for telemetry data transmission. The firmware implementation includes connection handling with automatic reconnection, periodic telemetry publishing using FreeRTOS tasks, and bidirectional RPC communication for remote device control. The dashboard was configured with multiple widget types including Digital Gauges for real-time sensor readings, Charts for historical trends, RPC Switches for remote device control, and an Open Street Map for geographical visualization. These components form the foundation for the monitoring and control system that will be expanded in subsequent chapters.
